...
interface MenuContextType {
  ...
  // Added toggleAvailability property to MenuContextType
...
export const MenuProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [foodItems, setFoodItems] = useState<FoodItem[]>(() => {
    ...
    // Ensure all items have isAvailable property initialized
  ...
  const addFoodItem = (item: Omit<FoodItem, 'id'>) => {
    const newItem: FoodItem = { ...item, id: `item-${Date.now()}`, isAvailable: true };
  ...
  // Implement toggleAvailability to update the item's availability state
