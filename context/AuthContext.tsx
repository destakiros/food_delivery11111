...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addUser = (user: Omit<User, 'id' | 'status' | 'notifications'>) => {
    const newUser: User = { ...user, id: `u-${Date.now()}`, status: 'Active', notifications: [] };
  ...
  const suspendUser = (id: string, until: string, reason: string) => {
    const message = `Account suspended until ${until}. Reason: ${reason}`;
  ...
  const liftSuspension = (id: string) => {
    const message = `Your suspension has been lifted. Welcome back!`;
    setUsers(prev => prev.map(u => {
      if (u.id === id) {
        const notif: AppNotification = { id: Date.now().toString(), message, timestamp: new Date().toLocaleString(), read: false };
  ...
  const addNotification = (userId: string, message: string) => {
    const notif: AppNotification = { id: Date.now().toString(), message, timestamp: new Date().toLocaleString(), read: false };
    setUsers(prev => prev.map(u => u.id === userId ? { ...u, notifications: [notif, ...u.notifications] } : u));
    if (currentUser?.id === userId) setCurrentUser(prev => prev ? { ...prev, notifications: [notif, ...prev.notifications] } : null);
  };
  ...
  const markNotificationsRead = () => {
    ...
    setUsers(prev => prev.map(u => u.id === currentUser.id ? { ...u, notifications: updated } : u));
  };
