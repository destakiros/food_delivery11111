...
export const MenuPage: React.FC<MenuPageProps> = ({ onNavToLogin, onViewTray, onTrackOrder, initialHistory = false }) => {
  ...
  const handleAddToCartAttempt = (item: FoodItem, size: string, price: number, exclusions: string[]) => {
    if (!currentUser) {
      alert("🛑 Access Denied: Please sign in to join our kitchen.");
      onNavToLogin();
      return;
    }
  ...
  const handleCheckout = () => {
    ...
    if (cartTotal < 5) {
      alert("🛑 Minimum Order: Please add at least $5.00 worth of food to your tray.");
      return;
    }
                ...
                <div key={order.id} className="bg-white dark:bg-zinc-900 rounded-[40px] border dark:border-zinc-800 shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-500">
                  <div className="flex flex-col md:flex-row">
                    <div className="p-10 flex-grow">
                      ...
                      <div className="space-y-3 mb-8">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-4 font-bold text-gray-700 dark:text-gray-300 uppercase italic text-sm">
                      ...
                      <div className="flex gap-4">
                        ...
                        {order.status === 'Delivered' && !order.review && (
                          <button 
                            ...
                            className="bg-ino-yellow text-red-900 px-8 py-3 rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-yellow-400 transition"
                          >
                    ...
                    <div className="md:w-64 bg-gray-50 dark:bg-zinc-800/50 p-10 flex flex-col justify-center items-end border-l dark:border-zinc-800">
                      <span className="text-[9px] font-black text-gray-400 uppercase mb-1">Total Valuation</span>
                      <div className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter italic">${order.totalPrice.toFixed(2)}</div>
                    </div>
                  </div>
                </div>
              ))
