...
interface OrderContextType {
  orders: Order[];
  placeOrder: (user: User, items: CartItem[], total: number) => void;
...
export const OrderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const placeOrder = (user: User, items: CartItem[], total: number) => {
      ...
      if (o.id === orderId) {
        return {
          ...
          message: status === 'Approved' ? 'Approved' : 
                   status === 'Cancelled' ? 'order cancelled' : `Status: ${status}`
        };
      }
      return o;
    }));
  ...
  const updateOrderReview = (orderId: string, review: Review) => {
    ...
    showToast("Intelligence received. Thank you, Operative.", "success");
  };
